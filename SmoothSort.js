...
const LP = [
  ...
  35421, 57313, 92735, 150049, 242785, 392835,
...
const sift = (arr, pshift, head, comparator) => {
  ...
  while (pshift > 1) {
    ...
    if (
      comparator(currNode, arr[lf]) >= 0 &&
      comparator(currNode, arr[rt]) >= 0
    ) {
      break;
    }
...
const trinkle = (arr, bitmap, pshift, head, isTrusty, comparator) => {
  ...
  while (bitmap != 1) {
    ...
    if (!isTrusty && pshift > 1) {
      ...
      let lf = head - 1 - LP[pshift - 2];
      if (
        comparator(arr[rt], arr[stepson]) >= 0 ||
...
module.exports = SmoothSort = (arr, comparator = defComparator) => {
  ...
  while (head < arr.length - 1) {
    log(
      `[Insert]  Rep:(${bitmap.toString(2)}|${pshift}), TreeSize:(${sizeOfTree(
        bitmap << pshift
      )}), Arr:[${arr.slice(0, head)}] <- [${arr[head]}]`
    );
    if ((bitmap & 3) == 3) {
      // combine 2 stretch
      sift(arr, pshift, head, comparator);
  ...
  while (pshift != 1 || bitmap != 1) {
    log(
      `[Dequeue]  Rep:(${bitmap.toString(2)}|${pshift}), TreeSize:(${sizeOfTree(
        bitmap << pshift
      )}), Arr:[${arr}]`
    );
    if (pshift <= 1) {
